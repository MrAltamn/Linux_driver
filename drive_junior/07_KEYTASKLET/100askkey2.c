...
void tasklet_irq_handle(unsigned long data)
{
	struct w100askkey2_dev *pmydev = (struct w100askkey2_dev *)data;
	int status1 = 0;
	int status2 = 0;
	int status = 0;
...
int __init w100askkey2_init(void)
{
	...
	tasklet_init(&pgmydev->tsk, tasklet_irq_handle, (unsigned long)pgmydev);
